...
@register('ranking_negative_heli_reader')
class RankingHeliReader(DatasetReader):
    
    def read(self, data_path):
        # data_path = expand_path(data_path)
        # self.download_data(data_path)
        dataset = {'train': None, 'valid': None, 'test': None}
        train_fname = Path(data_path) / 'heli_train.csv'
        valid_fname = Path(data_path) / 'heli_test.csv'
        test_fname = Path(data_path) / 'heli_test.csv'
        self.sen2int_vocab = {}
    ...
    def _build_sen2int_classes_vocabs(self, train_fname, valid_fname, test_fname):
        ...
        with open(train_fname, 'r') as f:
            ...
            for el in reader:
                sen_train.append(self.clean_sen(el[0]).lower())
                label_train.append(int(el[1]))
        with open(valid_fname, 'r') as f:
            reader = csv.reader(f, delimiter='\t')
            for el in reader:
                sen_valid.append(self.clean_sen(el[0]).lower())
                label_valid.append(int(el[1]))
        with open(test_fname, 'r') as f:
            reader = csv.reader(f, delimiter='\t')
            for el in reader:
                sen_test.append(self.clean_sen(el[0]).lower())
